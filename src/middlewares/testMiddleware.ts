...
export async function validateIds(req: Request, res: Response, next: NextFunction) {
  const { categoryId, disciplineId, teacherId } = req.body;
    ...
    const teacherDiscipline = await teacherDisciplineRepository.findByDataId({teacherId, disciplineId});
